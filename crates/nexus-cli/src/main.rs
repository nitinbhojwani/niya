//! Nexus CLI — the entry point for the interactive coding agent.
//!
...
use anyhow::Result;
use clap::Parser;
use nexus_core::ProviderAdapter;
use std::path::PathBuf;

/// Nexus — an AI coding assistant for your terminal.
#[derive(Parser, Debug)]
...
async fn main() -> Result<()> {
    ...
    let project_root = match &cli.project {
        ...
        None => {
            ...
            nexus_config::loader::detect_project_root(&cwd)
        }
    };

    let mut config = nexus_config::loader::load_config(&project_root)?;
    ...
    nexus_config::schema::validate(&config).map_err(|errors| {
        anyhow::anyhow!(
    ...
    let adapter = nexus_providers::OpenAiCompatAdapter::new(
        provider_name,
        provider_config
            .base_url
            .as_deref()
    ...
    let permission_gate =
        nexus_core::PermissionGate::new(config.permissions.clone(), &project_root);
    ...
    let session_logger = nexus_core::SessionLogger::new(&session_id, Some(&log_dir));

    // Build the orchestrator
    let orch_config = nexus_core::OrchestratorConfig {
        max_iterations: config.session.max_iterations,
        dry_run: cli.dry_run,
    };

    let tool_context = nexus_core::ToolContext {
        project_root: project_root.clone(),
        cwd: project_root.clone(),
        env: std::env::vars().collect(),
    };

    let orchestrator = nexus_core::Orchestrator::new(
        Box::new(adapter),
        tool_registry,
        permission_gate,
        context_manager,
